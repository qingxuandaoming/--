...
**初始系统管理员账号：**
- 用户名：`root`
- 密码：`YOUR_DB_PASSWORD_HERE`

### 💻 手动分步启动 (或使用本地一键脚本)

如果您不使用 Docker，可以使用以下方式本地启动：

**推荐：使用本地一键启动脚本**
双击项目根目录下的 `start_local.bat`，脚本将自动为您开启 3 个命令行窗口，分别运行 Java 后端、Python 后端和 Vue 前端（请确保您已经启动了本地的 MySQL，并且配置好了所需的开发环境：Java、Python、Node）。
