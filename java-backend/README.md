...
灵境行者路径规划后端服务是一个基于Spring Boot开发的路径规划API服务，集成高德地图API，为前端应用提供路径规划、地理编码等功能。

## 功能特性

- 🚗 **多种交通方式路径规划**：支持驾车、步行、公交、骑行四种交通方式
- 🗺️ **地理编码服务**：地址与坐标相互转换
- 🎯 **多种路径策略**：速度优先、费用优先、距离优先等多种策略
- 🚀 **高性能缓存**：内置缓存机制，提升响应速度
- 🔒 **参数验证**：完善的请求参数验证和异常处理
- 📊 **详细路径信息**：提供路径步骤、距离、时长、费用等详细信息
- 🌐 **跨域支持**：支持前端跨域访问
