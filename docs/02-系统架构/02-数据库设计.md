...
# 执行备份
mysqldump -u root -pYOUR_DB_PASSWORD_HERE \
  --single-transaction \
  --routines \
  --triggers \
  --events \
...
# 刷新日志
mysql -u root -pYOUR_DB_PASSWORD_HERE -e "FLUSH LOGS;"
...
# 创建恢复数据库
mysql -u root -pYOUR_DB_PASSWORD_HERE -e "CREATE DATABASE IF NOT EXISTS $TARGET_DB;"

# 恢复数据
if [[ $BACKUP_FILE == *.gz ]]; then
    gunzip -c $BACKUP_FILE | mysql -u root -pYOUR_DB_PASSWORD_HERE $TARGET_DB
else
    mysql -u root -pYOUR_DB_PASSWORD_HERE $TARGET_DB < $BACKUP_FILE
fi
...
# 检查数据库连接
mysql -u root -pYOUR_DB_PASSWORD_HERE -e "SELECT 1;" > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "ERROR: Cannot connect to MySQL" | mail -s "DB Alert" admin@ljxz.com
fi

# 检查表空间使用率
USAGE=$(mysql -u root -pYOUR_DB_PASSWORD_HERE -e "SELECT ROUND(SUM(data_length + index_length) / 1024 / 1024, 2) AS 'DB Size in MB' FROM information_schema.tables WHERE table_schema='ljxz';" | tail -1)
...
# 检查慢查询
SLOW_QUERIES=$(mysql -u root -pYOUR_DB_PASSWORD_HERE -e "SHOW GLOBAL STATUS LIKE 'Slow_queries';" | tail -1 | awk '{print $2}')
...
### 技术文档
- [架构设计文档](./01-系统架构设计.md) - 系统架构设计
- [开发指南](../03-开发指南/01-开发规范与流程.md) - 开发环境搭建和规范
- [部署文档](../05-部署运维/01-部署指南.md) - 生产环境部署指南
- [安全文档](../03-开发指南/03-安全指南.md) - 安全策略和最佳实践
